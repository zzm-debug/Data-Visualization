...
(function () {
  ...
  function loadData() {
    ...
    d3.json("src/assets/歌单_2314343014_genre分布统计_归类后.json").then(function (data) {
  ...
  function init3DHeatmap() {
    ...
    if (genreData && genreData.length > 0) {
      for (var _i2 = 0; _i2 < genreData.length; _i2++) {
        ...
        if (chunk && chunk.genre_distribution) {
          var _iterator = _createForOfIteratorHelper(allGenres),
            _step;
          try {
    ...
    if (!genreData || genreData.length === 0 || minLineHeight === Infinity) {
      ...
      maxLineHeight = 0;
      for (var _i3 = 0; _i3 < surfaceData.length; _i3++) {
        for (var _j = 0; _j < surfaceData[_i3].length; _j++) {
          var _val = surfaceData[_i3][_j] || 0;
    ...
    allGenres.forEach(function (genre, idx) {
      ...
      g.append("text").attr("class", "axis-label x-label x-label-".concat(idx)).attr("x", projected.x).attr("y", projected.y).attr("text-anchor", "middle").attr("fill", "#333").attr("font-size", "11px").attr("font-weight", "500").style("font-family", "system-ui, -apple-system, sans-serif").text(genre);
    });
    ...
    months.forEach(function (month, idx) {
      if (idx % 3 === 0) {
        ...
        g.append("text").attr("class", "axis-label y-label y-label-".concat(idx)).attr("x", projected.x).attr("y", projected.y).attr("text-anchor", "end").attr("fill", "#333").attr("font-size", "10px").style("font-family", "system-ui, -apple-system, sans-serif").text(month);
      }
    });

    // Z轴标签（乐曲数量）- 再往上移
    var zLabelPos = isometricProjection(-10, 0, maxHeight + 25); // 增加Z坐标，让标签更往上
    g.append("text").attr("class", "axis-label z-label").attr("x", zLabelPos.x).attr("y", zLabelPos.y).attr("text-anchor", "middle").attr("fill", "#333").attr("font-size", "10px").attr("font-weight", "500").style("font-family", "system-ui, -apple-system, sans-serif").text("乐曲数量");
...
//# sourceMappingURL=/heatmap.13f5bbab.js.map