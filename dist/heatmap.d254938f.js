...
(function () {
  ...
  function init3DHeatmap() {
    ...
    var _loop = function _loop(_monthIdx) {
      ...
      var lineColor = "rgba(10, 5, 20, 0.9)"; // 极深紫黑，接近纯黑（原图折线观感）
      // ✅ 恢复“折线下方半透明填充”：原图是浅紫色带状半透明
      var areaFillColor = "rgba(233, 213, 255, 0.55)"; // #e9d5ff with alpha
      ...
      if (lineData.length > 0) {
        ...
        if (areaPath && !areaPath.includes('NaN')) {
          var areaElement = g.append("path").attr("class", "area-path area-".concat(_monthIdx)).attr("d", areaPath).attr("fill", areaFillColor).attr("fill-opacity", 0.42).attr("stroke", "none").datum({
            lineData: lineData,
            month: month,
            monthIdx: _monthIdx
          }) // 存储数据
...
//# sourceMappingURL=/heatmap.d254938f.js.map