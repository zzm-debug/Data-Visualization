...
(function () {
  ...
  //#region Heatmap 3D｜主题/装饰层（统一主页面与新窗口大图的“截图风格”）
  ...
  function getCyberStops() {
    ...
    // ✅ 修正颜色：去除青色，改用纯粹的 紫->粉->白 渐变，贴合截图
    return ["#0f0c29", "#302b63", accentViolet, accentPink, "#ffffff"];
  }
  function ensureHeatmapOverlay(modeText) {
  ...
  // ✅ 给 `heatmap-toggle.js` 一个“安全的徽标更新入口”（避免 2D/3D 脚本加载顺序导致徽标显示不一致）
  window.updateHeatmapOverlayMode = function (mode) {
    try {
      var stage = container && (container.closest(".heatmap-stage") || container.parentElement);
      var badge = stage ? stage.querySelector(".hm-badge") : null;
      if (badge && mode) badge.textContent = mode;
    } catch (e) {}
  };
  //#endregion
  ...
  function init3DHeatmap() {
    ...
    var lineGlow = defs.append("filter").attr("id", "hm-line-glow").attr("x", "-30%").attr("y", "-30%").attr("width", "160%").attr("height", "160%");
    lineGlow.append("feGaussianBlur").attr("in", "SourceGraphic").attr("stdDeviation", "2.2").attr("result", "lineBlur");
    var lineMerge = lineGlow.append("feMerge");
    ...
    var _loop = function _loop(_monthIdx) {
      ...
      // ✅ 修正：原先 darkenColor(..., 0.16) 会把线条“压到接近黑”，和截图的霓虹线不符
      var areaFillColor = darkenColor(lineColor, 0.62);
      ...
      if (lineData.length > 0) {
        ...
        if (areaPath && !areaPath.includes('NaN')) {
          var areaElement = g.append("path").attr("class", "area-path area-".concat(_monthIdx)).attr("d", areaPath).attr("fill", areaFillColor).attr("fill-opacity", 0.28).attr("stroke", "none").datum({
            lineData: lineData,
            month: month,
            monthIdx: _monthIdx
          }) // 存储数据
        ...
        if (linePath && !linePath.includes('NaN')) {
          var lineElement = g.append("path").attr("class", "line-path line-".concat(_monthIdx)).attr("d", linePath).attr("fill", "none").attr("stroke", lineColor).attr("stroke-width", 2.8).attr("opacity", 1).attr("filter", "url(#hm-line-glow)").datum({
            lineData: lineData,
            month: month,
            monthIdx: _monthIdx
          }) // 存储数据
          ...
          if (maxPoint) {
            ...
            // 绘制外圈（白色，更大）
            g.append("circle").attr("class", "max-point-outer max-point-outer-".concat(_monthIdx)).attr("cx", maxProjected.x).attr("cy", maxProjected.y).attr("r", 6).attr("fill", "#ffffff").attr("stroke", lineColor).attr("stroke-width", 2).attr("opacity", 1).style("pointer-events", "none"); // 允许鼠标事件穿透到折线

            // 绘制内圈（折线颜色，更小）
            g.append("circle").attr("class", "max-point-inner max-point-inner-".concat(_monthIdx)).attr("cx", maxProjected.x).attr("cy", maxProjected.y).attr("r", 3.5).attr("fill", lineColor).attr("stroke", "none").attr("opacity", 1).style("pointer-events", "none"); // 允许鼠标事件穿透到折线
          }
...
//# sourceMappingURL=/heatmap.18bdc7f9.js.map